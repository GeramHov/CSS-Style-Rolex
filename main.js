...
let qtyValue = document.querySelector("input").value;

btn.addEventListener("click", function () {
  // let i = prompt(`Please choose desired quantity (1-10)`);
  let globalPrice = 43340 * qtyValue;
  if (qtyValue > 0 && qtyValue < 11) {
    alert(
      `Thank You for Your purchase \nYou have choosen ${qtyValue}x Rolex Cosmograph Daytona` +
        "\n" +
        `Total price: ${globalPrice} $`
    );
  } else {
    alert(
      "Sorry We could not provide more than 10 items" + "\n" + "See You soon"
    );
  }
});
